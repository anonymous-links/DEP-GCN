...
for train_loader, val_loader in zip(train_loaders, val_loaders):
    ...
    for epoch in range(0, config_training['epochs']):
        ...
        if early_stopping0([sampler, model], val_outputs[1], save_sampler, epoch+1):
            ...
            edge_reduction = early_stopping0._store_criteria['edge_reduction'] # TO DO store criteria needs to be a dict with all info on that epoch.... 
    ...
    torch.save(sampler.state_dict(), f"/path/fDEP_{fold}.pth")
