...
config_gcn = ParameterGrid({'num_layers': [4], 
                            'hidden_dim': [256], 
                            'edge_attr': [None], # 1, None: for unweighted graphs = None
                            'activ_funct': [nn.ReLU()],
                            'dropout': [0.5], 
                            'final_dropout': [None], 
...
# Dataset 
dataset_name = 'PearC_EdgeW_PearC_Sp_fully_connected_raw_Sex' 
...
def evaluate(loader, sampler, model, criterion ):
    ...
    #y_bin = (y_pred >0.5).astype(np.float32) # for binary
...
for train_loader, val_loader in zip(train_loaders, val_loaders):
    ...
    for epoch in range(0, config_training['epochs']):
        ...
        save_sampler = {'edge_reduction': edge_reduction, 'train_loss': train_outputs[1],
                        'val_loss': val_outputs[1],'train_f1': f1score_train, 
                        'val_f1': f1score_val, 'train_acc': acc_train, 
                        'val_acc': acc_val
                        }
        ...
        if early_stopping0([sampler, model], val_outputs[1], save_sampler, epoch+1):
            ...
            early_stopping0.restore_best_model([sampler, model]) # restore best DEP: with weight mask from best iteration
            edge_reduction = early_stopping0._store_criteria['edge_reduction'] 
            best_epoch = early_stopping0._epoch
            break

        if (epoch % config_sampler['ite_step'] ==0) & (epoch!=0): # epoch to update prune p% ?

            if (edge_reduction >= 90) & (edge_reduction < 99): # change step p% if edge reduction [90,99]
                step = 0.01

            elif (edge_reduction >= 99) & (edge_reduction != 100): # change step p% if edge reduction [99,100]
                step = 0.001

            elif edge_reduction == 100: # stop if edge reduction achieved 100% and return best prior model
                early_stopping0.early_stop = True
                early_stopping0.restore_best_model([sampler, model])
                edge_reduction = early_stopping0._store_criteria['edge_reduction'] # best sp level associated with best weight_mask also needs to be restore
                best_epoch = early_stopping0._epoch
                break        

            with torch.no_grad(): # at the end, if cycle is not broken, update prune sp% with step increase

                sampler.update_prune(step)
        
############################################## RE-TRAIN MODEL #####################################################################################################################

# Re train the model from scratch with the best subnetwork found by DEP sampler
    
    model1 = GCN(config_gcn, input_dim=node_feat_dim, output_dim=output_dim)
    model1 = model1.to(device)
    sampler.min_sp = edge_reduction /100 # DEP has been restore with best weight_maks but this will be prune according associated sp level
    ...
    for epoch in range(0, config_training['epochs']):
        ...
        for data in train_loader:
                ...
                out = model1(sampled_data.x, sampled_data.edge_index, sampled_data.batch)
                labels = torch.nn.functional.one_hot(data.y, num_classes=2) # for cross_entropy
                loss_clf = criterion(out, labels.float())
    ...
    results_filename = '/path_save_results/training_results.csv'
    ...
    # Check if file exists to determine whether to write header
    wtable_filename = '/path_save_results/weights_table.csv'
    ...
    torch.save(sampler.state_dict(), f"/path_save_results/DEP_{fold}.pth")
    torch.save(model.state_dict(), f"/path_save_results/GCN_{fold}.pth")