...
class EarlyStopping:
    ...
    def _get_model_copy(self, model):
        ...
        #model_copy = model.__class__.__new__(model.__class__) # Create a deep copy of the model class
        #model_copy.load_state_dict(model.state_dict())  # Copy the model's weights
        ...
        if isinstance(model, list):
            ...
            for modeli in model: # if we need to save more than 1 best model
                ...
                if hasattr(modeli.lin, "weight_orig"): 
                    #prune.remove(model.lin, "weight")  # Remove old pruning mask  
                    modeli.lin.weight_orig.data.copy_( modeli.lin.weight) 
        ...
        if isinstance(self._best_model_weights, list):
            ...
            for modeli, best_model_weights in zip(model, self._best_model_weights):
                ...
                if hasattr(modeli.lin, "weight_orig"):
                    ...
                    prune.remove(modeli.lin, "weight")  
